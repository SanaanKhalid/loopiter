---
title: Migrating from the prototype
---

This page preserves the historical 0.1/branding → 0.2 migration. For the new controller,
store contracts and explicit 0.2 → 0.3 upgrade, use [Migrating to 0.3](/operate/migrating-to-0-3).

## Feloop to Loopiter

Install `loopiter@0.2.0-alpha.1`. Change imports to
`loopiter`, `loopiter/postgres` and `loopiter/testing`,
the CLI to `loopiter`, and `FeloopError` to `LoopiterError`. Rename starter and test
environment variables from `FELOOP_*` to `LOOPITER_*`.

Keep your database and namespaces. Legacy SQL table names and advisory-lock keys
are intentionally unchanged to preserve data and cross-version coordination.
Existing service URLs and historical release reports also retain their old identities.
